    ...
    def __init__(self, num_agents, imposters):
        ...
        for i in range(num_agents):
            if(i not in imposters):
                relations[i] = set((x,y) for x in range(num_agents) for y in range(num_agents) if ((i is not x) and (i is not y)))
            else:
                relations[i] = set((x,y) for x in range(num_agents) for y in range(num_agents) if ((x is y) and (i is x)))

        relations.update(add_symmetric_edges(relations))

        self.kripke_structure = KripkeStructure(worlds, relations)
